...
export type StorySlide = {
  ...
  // optional: override the eyebrow label (defaults to slide_type)
  eyebrow?: string;
};
...
export type StoryCtaConfig = {
  primary: StoryCtaButton;
  secondary?: StoryCtaButton;
...
function toneClass(tone: StoryOverlay) {
  switch (tone) {
...
function toneBase(tone: StoryOverlay) {
  switch (tone) {
...
export function StoryViewer({
  ...
  const bodyText = clean(slide.body);
  const eyebrow = (slide.eyebrow ?? slide.slide_type).replace(/_/g, " ");
    ...
    <div className="fixed inset-0 z-[100] bg-cream md:bg-cream md:grid md:place-items-center">
      ...
      >
        ...
        <div className="relative z-10 h-full flex flex-col px-6 pt-16 pb-10 text-ink">
          <p className="brand-eyebrow lowercase tracking-[0.18em] text-[11px]">{eyebrow}</p>
          ...
          {slide.slide_type === "results" ? (
            ...
            <CtaSlide
              headline={clean(slide.headline) || cta.defaultHeadline || "ready when you are."}
              body={bodyText || cta.defaultBody || ""}
              cta={cta}
            />
          ) : (
...
function CtaSlide({
  ...
  return (
    <div className="mt-6 flex-1 flex flex-col">
      ...
      {body && (
        <p className="mt-3 text-[15px] lowercase text-ink">{body}</p>
      )}
      <div className="flex-1" />
      <button
        onPointerDown={(e) => e.stopPropagation()}
        onClick={(e) => { e.stopPropagation(); cta.primary.onClick(); }}
      ...
      {cta.secondary && (
        <button
          onPointerDown={(e) => e.stopPropagation()}
          onClick={(e) => { e.stopPropagation(); cta.secondary!.onClick(); }}
